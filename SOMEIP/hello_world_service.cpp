...
static vsomeip::instance_t service_instance_id2 = 0x5555;
static vsomeip::method_t service_method_id2 = 0x6666;
...
public:
    ...
    bool init()
    {
        ...
        app_->register_message_handler(service_id, service_instance_id2,
                service_method_id2,
                std::bind(&hello_world_service::on_message_cbk2, this,
                        std::placeholders::_1));
    ...
    void stop()
    {
        ...
        app_->stop_offer_service(service_id, service_instance_id2);
        // unregister the state handler
        app_->unregister_state_handler();
        // unregister the message handler
        app_->unregister_message_handler(service_id, service_instance_id,
                service_method_id);
        app_->unregister_message_handler(service_id, service_instance_id2,
                service_method_id2);
    ...
    void on_state_cbk(vsomeip::state_type_e _state)
    {
        if(_state == vsomeip::state_type_e::ST_REGISTERED)
        {
            ...
            app_->offer_service(service_id, service_instance_id2);
    ...
    void on_message_cbk(const std::shared_ptr<vsomeip::message> &_request)
    {
        ...
        std::cout << "Ghansham msg cbk ===>" << std::endl;
	...
	char * ch = reinterpret_cast<char*>(_request->get_payload()->get_data());
	//std::cout << "ghansham == " << ch ;
	//unsigned int x = (int &)(ch);
	//std::cout << "ghansham == " << sizeof(unsigned int) << "==" << x ;
    ...
    void on_message_cbk2(const std::shared_ptr<vsomeip::message> &_request)
    {
        std::cout << "Ghansham msg cbk2 ===>" << std::endl;
        // Create a response based upon the request
        std::shared_ptr<vsomeip::message> resp = rtm_->create_response(_request);

        // Construct string to send back
        std::string str("Ghansham ");
        str.append(
                reinterpret_cast<const char*>(_request->get_payload()->get_data()),
                0, _request->get_payload()->get_length());
